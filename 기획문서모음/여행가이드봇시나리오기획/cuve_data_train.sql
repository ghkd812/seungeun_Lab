...
insert into VGLog_2015
select *
from dbo.VGLog_2015
where SEQ_NO >=300000000 and IN_DATE >= '2015-01-01' and IN_DATE < '2016-01-01'


create index IDX_VGLog_2015
on VGLog_2015(IN_DATE, IN_IP)
include (IN_PATH, IN_QUERY, IN_TYPE1, IN_PRO_CODE, IN_MASTER_CODE);

create table TMP_RES_COMPLETE_LOG_2015(
	SEQ_NO int,
	IN_DATE datetime,
	IN_IP varchar(20),
	IN_PATH varchar(500),
...
insert into TMP_RES_COMPLETE_LOG_2015
select *
from VGLog_2015
...
create table TMP_RES_COMPLETE_LOG_2015_2(
	RES_LOG_SEQ int,
	IN_IP varchar(20),
	IN_DATE datetime,
	RES_WEEK varchar(20),
...
insert into TMP_RES_COMPLETE_LOG_2015_2
SELECT
...
(
	...
	FROM TMP_RES_COMPLETE_LOG_2015 a
...
create table TMP_RES_RESULT_PROCESS_LOG_2015(
	RES_LOG_SEQ int,
	IN_IP varchar(20),
	IN_DATE datetime,
	OPEN_WEEK int,
	SEQ int,
	OPEN_MASTER VARCHAR(100)
);
...
WITH LIST AS    -- 상품열람 횟수가 극단값에 해당하는 Data를 제거하기 위한 CTE문
	(
		...
		FROM VGLog_2017 A --2015년의 모든 로그가 담긴 테이블
		INNER JOIN TMP_RES_COMPLETE_LOG_2015_2 B -- [나]에서 분류한 실예약로그와 JOIN 
...
insert into TMP_RES_RESULT_PROCESS_LOG_2015
...
(
	...
	INNER JOIN TMP_RES_COMPLETE_LOG_2015_2 B ON A.IN_IP = B.IN_IP AND DATEPART(WEEK,A.IN_DATE) = B.RES_WEEK
...
INTO #TEMP
FROM 
TMP_RES_COMPLETE_LOG_2015_2 A
INNER JOIN TMP_RES_RESULT_PROCESS_LOG_2015 B ON A.RES_LOG_SEQ = B.RES_LOG_SEQ 
...
select * from TMP_RES_RESULT_APORIORI_TEST
...
---- 2. TEST DATASET 생성 x  
--=================================================================================================================
select * from TMP_2017_2_LOG_TEST_DATA
select * from TMP_RES_COMPLETE_LOG_2017_2

--가. 2016년 전체 로그테이블에서 예약완료페이지로만 조건설정 후 별도 테이블 생성

SELECT * 
INTO 
	TMP_RES_COMPLETE_LOG_2017 
FROM 
	TMP_2017_LOG_DATA --전체 로그테이블에서 2015년 로그만 별도 분리한 테이블
WHERE 
	IN_PATH LIKE '%Reserve/Package/Complete%' -- PC예약완료페이지 로그
--모바일예약완료로그 IN_PATH LIKE '%Mobile/Reserve/RegisterForm%' 


SELECT * FROM TMP_RES_COMPLETE_LOG_2017
...
SELECT * FROM TMP_2016_LOG_TEST_DATA2
ORDER BY RES_LOG_SEQ , [NO] ASC

--DROP TABLE TMP_2016_LOG_TEST_DATA2
...
select * from tmp_2016_log_data

select * from cus_clear_history order by cus_name
select * from cus_master_history order by cus_no, his_date
select * from TMP_RES_RESULT_APORIORI_TEST
select * from TMP_APORIORI_DATA
select * from TMP_RES_COMPLETE_LOG_2016_2